...
from cuda_de_biped import solve_footstep_planning_with_cuda, generate_footstep_and_com_trajectory
...
print("robot model", robot.model.name)
...
def move(trajectory_type:str,
    ...
    # x0_MLD = np.array([0.0, 0.0, -0.0, -0.0, 1.58432257])
    x0_MLD = np.array([ 0.29357406,  0.29125562, -0.01193462, -0.01774755,  1.58432257])
    N = 30

    sol_x, sol_u, fitness, solve_time = solve_footstep_planning_with_cuda(x0_MLD, N)
    ...
    print("len(left_foot)", len(left_foot))  # e.g. 120 or 160?
    print("len(right_foot)", len(right_foot))  # e.g. 120 or 160?
    print("len(com)", len(com))        # e.g. 160?
    
    # plt.plot([f[0] for f in left_foot if f], label="left foot x")
    # plt.plot([f[0] for f in right_foot if f], label="right foot x")
    # plt.plot([c[0] for c in com], label="com x")
    # plt.legend()
    # plt.show()
    ...
    print("left_foot_unique", left_foot_unique)
    print("right_foot_unique", right_foot_unique)
    print("left_foot_orientation", left_foot_orientation)
    print("right_foot_orientation", right_foot_orientation)
